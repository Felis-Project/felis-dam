...
class FelisDamPlugin : Plugin<Project> {
    ...
    abstract class Extension(@Inject private val project: Project, @Inject private val objects: ObjectFactory) {
        ...
        val include: Configuration = project.configurations.maybeCreate("include").apply {
            isTransitive = false
        ...
        project.tasks.withType(Jar::class.java) {
            it.doFirst {
                ...
                for (file in ext.include.files) {
                    val dest = jarDir.file(file.name).asFile
                    dest.createNewFile()
                    file.copyTo(dest, true)
                    jarPaths.add(file.name)
                }

                val jarString = jarPaths.joinToString(",") { path -> "\"$path\"" }

                val jarDataToml = """
                # This file has been automatically generated by felis-dam.
                # For more info look into the JarInJar system.
                jars = [ $jarString ]
                """.trimIndent()

                val jarData = jarDir.file("jars.data.toml").asFile
                jarData.createNewFile()
                jarData.writeText(jarDataToml)
            }
